/* ============================================================
   ...
   MONDE OUVERT jouable (façon napoleon-phaser.html), réskin Grèce :
   on se déplace librement (flèches/WASD ou croix tactile + A),
   on entre dans 5 monuments qui posent chacun une question d'histoire 6e.
   Tout exploré = quête réussie (+XP, avatar évolue).
   Module distinct : définit sa scène Phaser et s'enregistre via
   OdyssiaEngine.registerQuest(...).
   ============================================================ */
(function (root) {
  "use strict";
  const GAME_W = 480, GAME_H = 520, TILE = 32, MW = 22, MH = 16;
  ...
  /* ---- 5 monuments = 5 questions (Grèce antique, 6e) ---- */
  const GRECE = {
    maxXP: 40,
    intro: ["Bienvenue à Athènes, jeune voyageur ! Explore la cité avec les flèches (ou la croix), puis entre dans les monuments avec A.",
            "Visite les 5 lieux et réponds à leurs énigmes pour percer les secrets de la Grèce antique."],
    monuments: [
      { key:"agora",    name:"L'Agora",      emoji:"🏛️", q:"Une cité grecque indépendante comme Athènes s'appelle…", options:["une polis","une légion","un pharaon","une tribu"], answer:0, explain:"Une cité-État grecque s'appelle une polis : une ville et son territoire, avec ses lois." },
      { key:"pnyx",     name:"L'Assemblée",  emoji:"🗳️", q:"À Athènes, qui pouvait voter à l'Ecclésia (l'assemblée) ?", options:["Les citoyens : hommes libres nés à Athènes","Tout le monde, femmes et esclaves compris","Seulement le roi","Uniquement les enfants"], answer:0, explain:"Seuls les citoyens (hommes libres nés à Athènes) votaient." },
      { key:"acropole", name:"Le Parthénon", emoji:"🦉", q:"Le Parthénon, sur l'Acropole, est un temple dédié à la déesse…", options:["Athéna","Aphrodite","Héra","Artémis"], answer:0, explain:"Le Parthénon honore Athéna, protectrice d'Athènes (emblème : la chouette)." },
      { key:"olympie",  name:"Olympie",      emoji:"🔥", q:"Les Jeux Olympiques se tenaient à Olympie en l'honneur du dieu…", options:["Zeus","Poséidon","Hadès","Hermès"], answer:0, explain:"Les Jeux Olympiques célébraient Zeus, le roi des dieux." },
      { key:"theatre",  name:"Le Théâtre",   emoji:"🎭", q:"Au théâtre grec, on jouait surtout des tragédies et des…", options:["comédies","opéras","films","ballets"], answer:0, explain:"Le théâtre grec présentait des tragédies et des comédies." }
    ]
  };
  // positions fixes des bâtiments (w=3,h=2)
  ...
  /* ---- peintres de textures (réskin grec) ---- */
  function drawHero(g,dir,step){ const cx=14, lL=step===1?-2:0, lR=step===1?0:-2;
    g.fillStyle(0x000000,0.18); g.fillEllipse(cx,34,16,5);
    g.fillStyle(0xe8c9a0,1); g.fillRect(cx-5,28+lL,4,7); g.fillRect(cx+1,28+lR,4,7);            // jambes
    g.fillStyle(0xf4f0e6,1); g.fillRect(cx-6,15,12,14);                                          // tunique blanche
    g.fillStyle(0xddd6c4,1); g.fillRect(cx+2,15,4,14);
    g.fillStyle(0x3a6baf,1); g.fillRect(cx-6,15,12,3);                                           // bande bleue
    g.fillStyle(0xf1c9a5,1); g.fillRect(cx-5,6,10,11);                                           // tête
    g.fillStyle(0x6a4a2a,1); g.fillRect(cx-5,4,10,5);                                            // cheveux
    if(dir==="up"){ g.fillStyle(0x553a1f,1); g.fillRect(cx-5,4,10,12); } else if(dir==="right"){ g.fillStyle(0x6a4a2a,1); g.fillRect(cx-5,4,9,7); }
    g.fillStyle(0xfdd404,1); g.fillRect(cx-5,9,10,2);                                            // couronne d'olivier
  ...
  function drawTemple(g,w,h,accent){ const cx=w/2;
    ...
    g.fillStyle(0xcfc7b2,1); g.fillRect(2,h-8,w-4,8); g.fillRect(6,h-15,w-12,8);                 // socle
    const bw=Math.round(w*0.82), bx=cx-bw/2, by=h-15-Math.round(h*0.5), bh=Math.round(h*0.5);
    g.fillStyle(0xf4f0e4,1); g.fillRect(bx,by,bw,bh);                                            // mur
    ...
    g.fillStyle(accent,1); g.beginPath(); g.moveTo(bx-6,by); g.lineTo(cx,by-20); g.lineTo(bx+bw+6,by); g.closePath(); g.fillPath(); // fronton
    g.fillStyle(dk(accent,12),1); g.fillRect(bx-6,by-2,bw+12,4);
    const dw=Math.round(bw*0.26), dh=Math.round(bh*0.6); g.fillStyle(0x6a4a2a,1); g.fillRect(cx-dw/2,h-15-dh,dw,dh); // porte
  ...
  class GreceWorldScene extends Phaser.Scene {
    ...
    create(){
      ...
      this.makeTextures();
      this.cameras.main.setBackgroundColor(0x9fc06a);
      this.buildWorld();
    ...
    makeTextures(){ const mk=(key,w,h,fn)=>{ if(this.textures.exists(key)) return; const g=this.make.graphics({add:false}); fn(g); g.generateTexture(key,w,h); g.destroy(); };
      ...
      mk("g_cobble",TILE,TILE,drawCobble);
      mk("g_olive",30,40,drawOlive); mk("g_col",24,44,drawColumn); mk("g_rock",30,24,drawRock);
    ...
    buildWorld(){
      const gen=genWorld(20260606); this.grid=gen.grid; this.buildings=gen.buildings; this.fp=gen.fp; this.entities=[];
      ...
      this.buildings.forEach(b=> this.makeBuilding(b));
      this.makeNPC();
      ...
      this.pShadow=this.add.image(this.px*TILE+TILE/2,this.py*TILE+TILE-2,"g_water0").setVisible(false);
      ...
      this.cameras.main.setDeadzone(36,36); this.cameras.main.roundPixels=true;
    }
    ...
    buildControls(){
      const mkPad=(x,y,label,dir)=>{ const c=this.add.container(x,y).setScrollFactor(0).setDepth(5e6); const g=this.add.graphics();
        g.fillStyle(0xfcf8ff,0.92); g.fillRoundedRect(-21,-21,42,42,10); g.lineStyle(2,root.QC.outlineVariant,1); g.strokeRoundedRect(-21,-21,42,42,10);
        const t=this.add.text(0,0,label,{fontFamily:FONT_TITLE,fontSize:"20px",color:"#1b1b23",fontStyle:"bold"}).setOrigin(0.5); c.add([g,t]);
        c.setSize(46,46).setInteractive(new Phaser.Geom.Rectangle(-23,-23,46,46),Phaser.Geom.Rectangle.Contains);
      ...
      const bx=58, by=GAME_H-58; mkPad(bx,by-44,"▲","up"); mkPad(bx,by+44,"▼","down"); mkPad(bx-46,by,"◀","left"); mkPad(bx+46,by,"▶","right");
      const A=this.add.container(GAME_W-56,GAME_H-58).setScrollFactor(0).setDepth(5e6); const ag=this.add.graphics();
      ag.fillStyle(hx("#4343d5"),1); ag.fillCircle(0,0,30); ag.lineStyle(3,0xffffff,1); ag.strokeCircle(0,0,30);
      A.add([ag,this.add.text(0,-2,"A",{fontFamily:FONT_TITLE,fontSize:"22px",color:"#fff",fontStyle:"bold"}).setOrigin(0.5),this.add.text(0,13,"ENTRER",{fontFamily:FONT_BODY,fontSize:"8px",color:"#fff",fontStyle:"bold"}).setOrigin(0.5)]);
      A.setSize(64,64).setInteractive(new Phaser.Geom.Rectangle(-32,-32,64,64),Phaser.Geom.Rectangle.Contains); A.on("pointerdown",()=>this.action());
      // quitter
      const Q=this.add.container(GAME_W-26,26).setScrollFactor(0).setDepth(5e6); const qg=this.add.graphics();
      qg.fillStyle(0xfcf8ff,0.95); qg.fillRoundedRect(-15,-15,30,30,9); qg.lineStyle(2,root.QC.outlineVariant,1); qg.strokeRoundedRect(-15,-15,30,30,9);
      Q.add([qg,this.add.text(0,0,"✕",{fontFamily:FONT_TITLE,fontSize:"16px",color:"#1b1b23",fontStyle:"bold"}).setOrigin(0.5)]);
      Q.setSize(34,34).setInteractive(new Phaser.Geom.Rectangle(-17,-17,34,34),Phaser.Geom.Rectangle.Contains); Q.on("pointerdown",()=>this.close());
      // bandeau progression
      this.prog=this.add.text(GAME_W/2,16,"",{fontFamily:FONT_TITLE,fontSize:"13px",color:"#fff",fontStyle:"bold",stroke:"#1b1b23",strokeThickness:4}).setOrigin(0.5,0).setScrollFactor(0).setDepth(5e6);
      this.refreshProg();
      // bulle d'aide / prompt
      this.prompt=this.add.text(GAME_W/2,52,"",{fontFamily:FONT_BODY,fontSize:"12px",color:"#1b1b23",backgroundColor:"#fcf8ff",padding:{x:8,y:4},fontStyle:"bold"}).setOrigin(0.5,0).setScrollFactor(0).setDepth(5e6).setVisible(false);
    }
    ...
    action(){ if(this.busy){ if(this.dlg){ this.advanceDialogue(); } return; } const f=this.facing();
      const b=this.buildings.find(b=> b.door.x===f.x && b.door.y===f.y);
      if(b){ if(this.done[b.idx]){ this.toast("Monument déjà visité ✔"); return; } SFX.enter(); this.openQuestion(b); return; }
    ...
    showDialogue(lines){ this.busy=true; this.dlgLines=lines; this.dlgIdx=0; if(this.prompt) this.prompt.setVisible(false);
      if(!this.dlg){ this.dlg=this.add.container(GAME_W/2,GAME_H-70).setScrollFactor(0).setDepth(6e6);
        this.dlgBg=this.add.graphics(); this.dlgTxt=this.add.text(0,0,"",{fontFamily:FONT_BODY,fontSize:"14px",color:"#1b1b23",align:"center",wordWrap:{width:GAME_W-90}}).setOrigin(0.5);
    ...
    _renderDlg(){ const txt=this.dlgLines[this.dlgIdx]; this.dlgTxt.setText(txt); const w=GAME_W-50, h=Math.max(64,this.dlgTxt.height+44);
      this.dlgBg.clear(); this.dlgBg.fillStyle(0x000000,0.12); this.dlgBg.fillRoundedRect(-w/2+4,-h/2+5,w,h,16);
      this.dlgBg.fillStyle(hx(root.QC?"#fcf8ff":"#fcf8ff"),1); this.dlgBg.fillStyle(0xfcf8ff,1); this.dlgBg.fillRoundedRect(-w/2,-h/2,w,h,16);
      this.dlgBg.lineStyle(2,root.QC.outlineVariant,1); this.dlgBg.strokeRoundedRect(-w/2,-h/2,w,h,16);
      this.dlgTxt.setY(-6); this.dlgHint.setY(h/2-14); }
    ...
    /* ---- question d'un monument (overlay in-scène) ---- */
    openQuestion(b){ this.busy=true; if(this.prompt) this.prompt.setVisible(false); const QC=root.QC; const m=b.mon;
      const W=GAME_W,H=GAME_H; const ov=this.add.container(0,0).setScrollFactor(0).setDepth(7e6); this.qov=ov;
      const dim=this.add.rectangle(W/2,H/2,W,H,0x1b1b23,0.55).setInteractive(); ov.add(dim);
      const PX=24,PW=W-48,PT=70,ph=H-150; const bg=this.add.graphics();
      bg.fillStyle(0x000000,0.14); bg.fillRoundedRect(PX+5,PT+8,PW,ph,20); bg.fillStyle(QC.surface,1); bg.fillRoundedRect(PX,PT,PW,ph,20);
      ...
      const cx=W/2; let y=PT+26;
      ov.add(this.add.text(cx,y,m.emoji+"  "+m.name,{fontFamily:FONT_TITLE,fontSize:"18px",color:"#b9651f",fontStyle:"bold"}).setOrigin(0.5,0)); y+=34;
      const qt=this.add.text(cx,y,m.q,{fontFamily:FONT_TITLE,fontSize:"15px",color:"#1b1b23",fontStyle:"bold",align:"center",wordWrap:{width:PW-40}}).setOrigin(0.5,0); ov.add(qt); y+=qt.height+14;
      const els=[]; let locked=false; const expl=this.add.text(cx,0,"",{fontFamily:FONT_BODY,fontSize:"12.5px",color:"#464555",align:"center",wordWrap:{width:PW-40}}).setOrigin(0.5,0);
      m.options.forEach((opt,i)=>{ const oh=38, ow=PW-36; const c=this.add.container(cx,y+oh/2); const g=this.add.graphics();
        const draw=(f,s)=>{ g.clear(); g.fillStyle(f,1); g.fillRoundedRect(-ow/2,-oh/2,ow,oh,12); g.lineStyle(2,s,1); g.strokeRoundedRect(-ow/2,-oh/2,ow,oh,12); }; draw(QC.surfaceContainer,QC.outlineVariant);
        const t=this.add.text(-ow/2+14,0,opt,{fontFamily:FONT_BODY,fontSize:"14px",color:"#1b1b23",wordWrap:{width:ow-26}}).setOrigin(0,0.5); c.add([g,t]); ov.add(c);
        c.setSize(ow,oh).setInteractive(new Phaser.Geom.Rectangle(-ow/2,-oh/2,ow,oh),Phaser.Geom.Rectangle.Contains,{useHandCursor:true});
        ...
        c.on("pointerdown",()=>{ if(locked) return; locked=true; this.answered++;
          if(i===m.answer){ this.correct++; draw(0xc8f5d8,QC.green); SFX.good(); this.cameras.main.flash(140,242,153,74); }
          else { draw(0xffd9de,QC.error); els[m.answer]._draw(0xc8f5d8,QC.green); SFX.bad(); }
          expl.setText("💡 "+m.explain); cont.setVisible(true);
        }); c._draw=draw; els.push(c); y+=oh+8; });
      expl.setY(y+2); ov.add(expl); y+=44;
      const cont=this.add.container(cx,PT+ph-30); const cg=this.add.graphics(); cg.fillStyle(hx(ACCENT),1); cg.fillRoundedRect(-90,-20,180,40,14); cg.lineStyle(2,dk(hx(ACCENT),12),1); cg.strokeRoundedRect(-90,-20,180,40,14);
      cont.add([cg,this.add.text(0,0,"Continuer  →",{fontFamily:FONT_TITLE,fontSize:"15px",color:"#fff",fontStyle:"bold"}).setOrigin(0.5)]); ov.add(cont);
      cont.setSize(180,40).setInteractive(new Phaser.Geom.Rectangle(-90,-20,180,40),Phaser.Geom.Rectangle.Contains,{useHandCursor:true}); cont.setVisible(false);
      cont.on("pointerdown",()=>{ SFX.click(); this.done[b.idx]=true; if(b._star) b._star.setVisible(true); this.refreshProg(); ov.destroy(); this.qov=null;
        if(this.done.every(Boolean)) this.finish(); else this.busy=false; });
    }

    toast(msg){ if(this._t) this._t.destroy(); this._t=this.add.text(GAME_W/2,GAME_H-150,msg,{fontFamily:FONT_TITLE,fontSize:"13px",color:"#fff",backgroundColor:"#27ae60",padding:{x:12,y:7}}).setOrigin(0.5).setScrollFactor(0).setDepth(8e6);
    ...
    finish(){ this.busy=true; const total=Math.max(1,this.answered), earned=Math.round(this.correct/total*this.content.maxXP);
      ...
      SFX.win(); const W=GAME_W,H=GAME_H; const ov=this.add.container(0,0).setScrollFactor(0).setDepth(9e6);
      ...
      out.setSize(220,44).setInteractive(new Phaser.Geom.Rectangle(-110,-22,220,44),Phaser.Geom.Rectangle.Contains,{useHandCursor:true}); out.on("pointerdown",()=>this.close()); }

    close(){ this.scene.stop(); if(root.OdyssiaEngine._cfg && root.OdyssiaEngine._cfg.onClose) root.OdyssiaEngine._cfg.onClose(); }
  }
